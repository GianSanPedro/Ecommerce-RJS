...
Aplicación fullstack de comercio electrónico con backend en Node/Express y frontend en React. Incluye catálogo con CRUD, carrito con pagos vía Mercado Pago, chat en tiempo real y gestión de contactos. Diseño modular para persistir en memoria, archivos JSON o MongoDB. Las subidas admiten FTP o fallback local en `media`, y el carrito se guarda en `localStorage` hasta finalizar la compra o borrarlo.

## Resumen rápido
- **Backend**: Express, JWT (header `access-token`), Socket.IO, Joi, Mongoose opcional, upload con Multer (imágenes hasta 5MB) hacia FTP o carpeta `media`, integración Mercado Pago.
- **Frontend**: React 18 (CRA), HashRouter, Redux Toolkit, Axios (envía `access-token`), `@mercadopago/sdk-react`, Socket.IO client.
- **Seguridad**: Middleware JWT `guarda`; `soloAdmin` protege productos, contactos y upload; validaciones Joi (productos, contactos, pedidos, credenciales).
...
- **Uploads**: Si falla o no se configura FTP, las imágenes se sirven desde `media/` local y se devuelve una URL utilizable; el endpoint crea `media` si falta.
...
- Capas: routers/controladores traducen HTTP/Socket a servicios; servicios manejan negocio (validaciones, pagos, guardado de pedidos y carrito pendiente, upload, JWT); DAO/Model fabrican MEM/FILE/MONGODB; frontend gestiona rutas, estado, carrito, auth, chat y contactos.
- Seguridad: `guarda` valida token; `soloAdmin` exige `admin=true`; respuestas coherentes 400/401/403.
...
- **Visitante** (token invitado): navegar, buscar productos, agregar al carrito y enviar contacto. No puede pagar (Wallet exige login) ni chatear (el chat se muestra solo con login). No es admin.
- **Cliente registrado** (login con email/password): lo anterior y además pagar con Mercado Pago (Wallet), usar el chat y validar su token. Sin permisos de administración.
- **Administrador** (`admin=true` en token): todo lo del cliente, más CRUD completo de productos (vista Alta), subir imágenes, listar y borrar contactos. El carrito oculta el botón de compra para admin por diseño.
...
- **FTP/CDN**: completa `FTP_*` y opcional `CDN_BASE_URL`; si no, se usa `media`.
